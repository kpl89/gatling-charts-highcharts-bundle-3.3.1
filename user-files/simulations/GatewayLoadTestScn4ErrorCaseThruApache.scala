...
class GatewayLoadTestScn4ErrorCaseThruApache extends Simulation {

	val httpProtocol = http
		.baseUrl("http://orb-ac0-gat-001:19000")
		.inferHtmlResources()
	...
	val scn = scenario("WebFluxDemoGateway")
		...
		.exec(http("getWith1sLatency")
			...
			.headers(headers_0))
		.exec(http("getWith2sLatency")
			.get("/webFluxDemo/getWith2sLatency")
			.headers(headers_0))

	setUp(scn.inject(rampUsers(500000).during(1000))).protocols(httpProtocol)
}